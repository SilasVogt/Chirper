...
With `--write-config`, it updates the whisper.cpp command, model path, and GPU
backend in the existing config when the Chirper CLI is available. If the CLI is
not available yet, it falls back to writing a starter config only when the config
